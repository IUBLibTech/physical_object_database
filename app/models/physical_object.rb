class PhysicalObject < ActiveRecord::Base
  ...
  scope :memnon_unstaged_by_date_and_format, lambda { |date, format|
    PhysicalObject.joins(:digital_provenance).where(format: format,staging_requested: false).where("digital_provenances.digitizing_entity = '#{DigitalProvenance::MEMNON_DIGITIZING_ENTITY}'").where(datesql(date)).order("RAND()")
  }
