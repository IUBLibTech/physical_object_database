module InvoiceHelper
	...
	def parse_invoice(upload)
		Thread.new() {
			mutex.synchronize do
				mis = MemnonInvoiceSubmission.new(filename: upload.original_filename, submission_date: Time.now, successful_validation: false, error_message: nil, validation_completeness_percent: 0)
				mis.save
				msg = ""
				...
				unless headers['Object barcode'].nil? or headers['Preservation master File name'].nil?
					...
					mis.update_attributes(success: false, error_message: msg, validation_completeness_percent: 100)
					return
				end
				...
				if @already_billed.size > 0 or @not_found.size > 0 or @not_on_sda.size > 0
					msg = ActionView::Base.new('app/views').render(file: 'invoice/failures')
					MemnonInvoiceSubmission.new(filename: upload.original_filename, submission_date: Time.now, successful_validation: false, error_message: msg, validation_completeness_percent: 100).save
				else
					MemnonInvoiceSubmission.new(filename: upload.original_filename, submission_date: Time.now, successful_validation: false, validation_completeness_percent: 0).save
				end
