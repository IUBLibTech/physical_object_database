class PhysicalObjectsController < ApplicationController
  ...
  def contained
    ...
    if @start_date && @end_date
      @physical_objects = PhysicalObject.all.eager_load(:unit, :picklist, :box, :bin, :box_bin, :bin_batch, :box_batch).joins(:workflow_statuses).where("workflow_statuses.workflow_status_template_id IN (?) AND workflow_statuses.created_at >= ? AND workflow_statuses.created_at <= ?", contained_status_ids, @start_date, @end_date)
    else
